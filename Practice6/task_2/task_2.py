...
file_size = os.path.getsize("H:/data/[2]automotive.csv.zip")
memory_usage = df_info_chunk("H:/data/[2]automotive.csv.zip","memory_usage_no_optimize.json", chunksize=500_000)
...
flag_header = True
size = 0
chunk = pd.read_csv("H:/data/[2]automotive.csv.zip", usecols=lambda x: x in dtypes.keys(), dtype=dtypes,chunksize=500_000)
...
box(df_optimize, "isNew", "askPrice")
