...
def pie(df, column):
    ...
    small_groups = groups[groups < 0.05 * len(df)]
...
def draw_plots(df_optimize):
    histogram(df_optimize.loc[df_optimize['country'].isin(["USA", "Russia", "Italy", "UK", "Japan", "China", "Germany"])], "country")
    pie(df_optimize, "country")
    linear_graphics(df_optimize, "quality_of_education", "quality_of_faculty")
    box(df_optimize.loc[df_optimize['country'].isin(["USA", "Russia", "Italy", "UK", "Japan", "China", "Germany"])], "country", "score")
    correlation(df_optimize, ["citations", "publications", "influence", "national_rank"])

file_name = "H:/data/cwurData.csv"    
file_size = os.path.getsize(file_name)
memory_usage = df_info_chunk(file_name,"memory_usage_no_optimize.json", chunksize=500_000)
print(f"Size without optimize: {memory_usage['file_in_memory_size']} MB")

dtypes = {'world_rank': pd.StringDtype(),'institution': pd.StringDtype(),'country': pd.StringDtype(),'national_rank': pd.StringDtype(),'quality_of_education': pd.StringDtype(), 'alumni_employment': pd.StringDtype(),'quality_of_faculty': pd.StringDtype(), 'publications': pd.StringDtype(), 'influence': pd.StringDtype(), 'citations': pd.StringDtype(), 'score': pd.StringDtype()}
