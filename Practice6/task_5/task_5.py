...
def pie(df, column):
    ...
    small_groups = groups[groups < 0.02 * len(df)]
...
def draw_plots(df_optimize):
    histogram(df_optimize, "class")
    pie(df_optimize, "class")
    linear_graphics(df_optimize, "albedo", "rms")
    box(df_optimize, "class", "diameter")
    correlation(df_optimize, ["diameter", "om", "albedo", "rms"])

file_name = "H:/data/[5]asteroid.zip"    
